...
float32_t *getMaxPositiveValue(int64_t size, int64_t expSize)
{
	return &(float32_t){ 0, (1 << expSize) - 2, (1 << (size - expSize - 1)) - 1 };
}

float32_t *getMaxNegativeValue(int64_t size, int64_t expSize)
{
	return &(float32_t){ 1, (1 << expSize) - 2, (1 << (size - expSize - 1)) - 1 };
}

float32_t *getMinPositiveValue(int64_t size, int64_t expSize)
{
	return &(float32_t){ 0, -(size - expSize - 2), 0 };
}

float32_t *getMinNegativeValue(int64_t size, int64_t expSize)
{
	return &(float32_t){ 1, -(size - expSize - 2), 0 };
}

float32_t *getPositiveZero(void)
{
	return &(float32_t){ 0, 0, 0 };
}

float32_t *getNegativeZero(void)
{
	return &(float32_t){ 1, 0, 0 };
}

float32_t *getPositiveInf(int64_t expSize)
{
	return &(float32_t){ 0, (1 << expSize) - 1, 0 };
}

float32_t *getNegativeInf(int64_t expSize)
{
	return &(float32_t){ 1, (1 << expSize) - 1, 0 };
}

float32_t *getNaN(int64_t expSize)
{
	return &(float32_t){ 1, (1 << expSize) - 1, 1 };
}
...
void normalize(int64_t *mant, int64_t *exp, int64_t mantSize, int64_t expSize)
{
	...
	{
		(*exp)++;
//		int64_t shift = (1ll << mantSize) - *mant;
//		*mant <<= shift;
//		*exp -= shift;
...
float32_t *floatAdd(float32_t *x, float32_t *y, int64_t size, int64_t expSize, int64_t roundType)
{
	...
	if (diff > mantSize + 1)
	{
		...
		exp1 += maxExp - 1;
		return &(float32_t){ sign1, exp1, mant1 };
	}
	if (diff != 0)
	{
		mant1 <<= diff;
	...
	mant -= (1ll << mantSize);
	exp2 += maxExp - 1;
	return &(float32_t){ sign1, exp2, mant };
}

float32_t *floatSubtract(float32_t *x, float32_t *y, int64_t size, int64_t expSize, int64_t roundType)
{
...
float32_t *bigExponentValue(int64_t roundType, int64_t sign, int64_t size, int64_t expSize) {
	if (roundType == 0)
	{
		return sign == 1 ? getMaxNegativeValue(size, expSize) : getMaxPositiveValue(size, expSize);
	}
	if (roundType == 2) {
		return sign == 1 ? getMaxNegativeValue(size, expSize) : getPositiveInf(expSize);
	}
	if (roundType == 3) {
...
float32_t *smallExponentValue(int64_t roundType, int64_t sign, int64_t size, int64_t expSize) {
	if (roundType == 0)
	{
		return sign == 1 ? getNegativeZero() : getPositiveZero();
	}
	if (roundType == 2) {
		return sign == 1 ? getNegativeZero() : getMinPositiveValue(size, expSize);
	}
	if (roundType == 3) {
		return sign == 1 ? getMinNegativeValue(size, expSize) : getPositiveZero();
	}
	return sign == 1 ? getNegativeZero() : getPositiveZero();
}

float32_t *floatMultiply(float32_t *x, float32_t *y, int64_t size, int64_t expSize, int64_t roundType)
{
	if ((isZero(x) && (isPositiveInf(y, expSize) || isNegativeInf(y, expSize))) ||
		((isZero(y)) && (isPositiveInf(x, expSize) || isNegativeInf(x, expSize))) ||
		isNaN(x, expSize) || isNaN(y, expSize))
	{
		return getNaN(expSize);
	}
	...
	if (exp > maxExp - 1)
	{
		return bigExponentValue(roundType, sign, size, expSize);
	}
	if (exp <= -maxExp - mantSize)
	{
		return smallExponentValue(roundType, sign, size, expSize);
	}
	if (exp <= -maxExp + 1)
	{
	...
	return &(float32_t){ sign, exp, mant };
}

float32_t *floatDivide(float32_t *x, float32_t *y, int64_t size, int64_t expSize, int64_t roundType)
{
...
float32_t *makeOperation(float32_t *x, float32_t *y, char operation, int64_t size, int64_t expSize, int64_t roundType)
{
	if (operation == '*')
	{
		return floatMultiply(x, y, size, expSize, roundType);
	}
	if (operation == '/')
	{
		return floatDivide(x, y, size, expSize, roundType);
	}
	if (operation == '+')
	{
		return floatAdd(x, y, size, expSize, roundType);
	...
	return floatSubtract(x, y, size, expSize, roundType);
}
...
float32_t makeFloat(int64_t x, int64_t size, int64_t expSize)
{
	int64_t mantSize = size - 1 - expSize;
	return (float32_t){ x >> (size - 1), (x >> mantSize) & ((1ll << expSize) - 1), x & ((1ll << mantSize) - 1) };
}
...
int main(int argc, char *argv[])
{
	...
	if (mode == 1)
	{
		...
		float32_t result = makeFloat(x, size, expSize);
	...
	else
	{
		...
		float32_t x_float = makeFloat(x, size, expSize);
		float32_t y_float = makeFloat(y, size, expSize);
		writeFloatingPoint(&x_float, size, expSize);
		writeFloatingPoint(&y_float, size, expSize);
		float32_t *result = makeOperation(&x_float, &y_float, operation, size, expSize, rounding);
		writeFloatingPoint(result, size, expSize);
	}
	return 0;
}
