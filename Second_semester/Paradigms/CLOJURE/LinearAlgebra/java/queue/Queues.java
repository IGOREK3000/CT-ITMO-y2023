...
public final class Queues {
    ...
    /* package-private */ interface DedupModel extends QueueModel {
        default void dedup() {
            Object prev = null;
    ...
    /* package-private */ static final LinearTester<DedupModel> DEDUP = (tester, queue, random) -> queue.dedup();

    // === Distinct

    /* package-private */ interface DistinctModel extends QueueModel {
        default void distinct() {
            model().removeIf(Predicate.not(new HashSet<>()::add));
        }
    }

    /* package-private */ static final LinearTester<DistinctModel> DISTINCT = (tester, queue, random) -> queue.distinct();
